    ...
    public async Task<UserAccess> GetUserAccess(string email, string organizationName)
    {
        ...
        {    
            ...
            var user = await context.UserAccess
                .FirstOrDefaultAsync(ua => ua.Email == email && ua.OrganizationName == organizationName) ?? throw new KeyNotFoundException($"User access not found for email: {email}, organization: {organizationName}");
            return user;
        }
        catch (Exception ex)
        {
